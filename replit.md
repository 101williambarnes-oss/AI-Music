...
## Recent Changes
- 2026-03-27: Create Album dual-source - creators can now add songs to albums two ways: pick existing songs already on the platform OR upload new songs from their computer; two-tab interface ("My Songs on Platform" / "Upload from Computer"); new uploads include title, genre, and audio file; uploaded during album creation then added to album; both types shown in live album preview with numbered order
- 2026-03-27: Albums feature - creators can create albums, add their tracks to albums, and manage them from the creator dashboard; album listing page at /albums, individual album detail at /album/:id with track listing and Play Album button; create album page at /create-album with cover upload, title, description, and track selection; "Creator Albums" nav button in home page topbar; "Your Albums" section in creator dashboard with Download Album links; albums and albumTracks tables in schema; full CRUD API with proper session auth and track ownership validation
- 2026-03-26: Landing page redesign - 3-column layout (Top 25 left, New Songs center, New Creators + Trending right); persistent bottom player bar with play/pause, progress scrubbing, volume controls; tagline updated to "Hit Wave Media for Music Creators — AI Only Music Platform"; Studios button text updated; responsive: 2-col on tablet, 1-col on mobile; audio context extended with resume/setVolume/getVolume for player bar controls; PlayerBar component in App.tsx renders on all pages
- 2026-03-25: AI DJ William Allen - personalized voice intros for each song; OpenAI Assistant generates intro scripts, ElevenLabs TTS creates William's voice, audio uploaded to Cloudinary; intros auto-generated on upload and cached in djIntroUrl; plays once before each song (first listen only); upload form adds "About This Song" description + city/state fields (first upload only, remembered after); POST /api/tracks/:id/dj-intro endpoint for on-demand generation; separate intro audio element in audio player for seamless intro-to-song transition
- 2026-03-22: Jukebox page - standalone /jukebox page for bar/venue use; scrollable grid of all tracks with cover art, title, creator name, and play button on each tile; neon side light strips; "Now Playing" header; no navigation, no tabs, no bottom controls; dark cyberpunk theme; no floating home button
